<template>
	<div class="ui-card" >
		<div class="ui-card-header">
			<div class="ui-card-title">Inserer Genre</div>
		</div>
			...
			<form class="ui-form" @submit.prevent="submitForm" method="POST" novalidate="novalidate" > 
				<fieldset :disabled="busy">
					<div class="ui-field ui-my-3">
						<div class="ui-field-label" >Genre</div>
						<input type="text" class="ui-input ui-input-fluid" placeholder="Genre" v-model="T.genre" required/>
					</div>
	...
	export default {
	    ...
	    methods: {
	      	submitForm: async function (e) {
	      		const [ok,message] = await api_fetch(this,'post',config.URL_GENRE,this.T,'ok');
	      		if (ok)
	      			setTimeout(()=>{this.$router.push('/genre')},1000);
	    	}
	    },
	};
</script>
